...
Interactive web application for analyzing educational inequalities using OECD PISA data (2012, 2015, 2018, 2022).

**Live Demo:** [kevinschoenholzer.com/pisa/](https://kevinschoenholzer.com/pisa/) (coming soon)

[![Status](https://img.shields.io/badge/status-complete-success)](https://github.com/kevisc/kevisc.github.io)
...
- 📊 **Progressive Data Loading** - Only fetches selected country-year combinations (~30-40 MB typical vs 1 GB full dataset)
...
- **Years**: 2012, 2015, 2018, 2022 (4 PISA cycles)
- **Countries**: 101 unique countries/economies
- **Students**: ~690,000 total (2022 cycle alone)
...
The data generation process creates 320 country-year JSON files (~1 GB total) from the learningtower R package.
...
- **Phase 1** (Data Pipeline): R scripts generate 320 country-year JSON chunks
...
│   │   ├── regression.js           # OLS, FE, RE models (448 lines)
...
│   │   ├── regression-viz.js       # Regression tables & plots (162 lines)
...
│   └── app.js                      # Main initialization (379 lines)
├── data/
│   ├── metadata.json               # Data catalog (generated by R)
│   └── country-year/               # 320 JSON files (~1 GB total)
│       ├── USA_2012.json           # ~3 MB
│       ├── USA_2015.json
│       ├── USA_2018.json
│       ├── USA_2022.json
...
**Problem Solved:** Original offline version required uploading 180 MB - 1 GB files upfront.

**Solution:** Pre-generated 320 country-year JSON chunks (2-5 MB each) that load on-demand.
...
// Total loaded: ~30-40 MB instead of 1 GB
```
...
- Regression tables with SE, t-stats, p-values
...
- **Participating Countries**: 101 countries across 4 cycles
...
Generates 320 country-year JSON files from learningtower package.
...
3. For each combination:
   ...
   - Write to file
4. Progress reporting (e.g., "Processing 47/320: USA 2018")

**Output:**
- ~320 files in `pisa/data/country-year/`
...
**Completeness:**
- All 4 years included (2012, 2015, 2018, 2022)
...
- **Cause:** Data files >50 MB individually, or repo >1 GB total
- **Solution:**
  1. Check individual JSON file sizes (should be 2-5 MB each)
  2. Total ~1 GB is at GitHub's soft limit but should work
...
**R Package:** learningtower package by Belinda Vaughan, Lydia Stanke, Tiffany Teng, Rob Hyndman, and Earo O'Hara-Wild

**Built With:** Vanilla JavaScript, Plotly.js, jStat, simple-statistics

**Inspired By:** The need for accessible tools for analyzing educational inequality in the quantitative sociology of education research community.

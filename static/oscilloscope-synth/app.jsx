...
function Slider({ label, min, max, step, value, onChange, format, disabled }){
...
function useTimeScope(analyserRef, canvasEl, cfg) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if(cfg && cfg.ac){ let s=0; for(let i=0;i<N;i++) s+=buf[i]; const m=s/N; for(let i=0;i<N;i++) buf[i]-=m; }
      const gain = (cfg && typeof cfg.gain==='number') ? cfg.gain : 1;
      const lw = (cfg && typeof cfg.lineWidth==='number') ? cfg.lineWidth : 2;
      ctx2d.strokeStyle="#6ef1a6"; ctx2d.lineWidth=lw; ctx2d.globalCompositeOperation="lighter";
